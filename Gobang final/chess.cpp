...
{
    const int weight[17] = {0, 10000000, -100000000, 50000, -100000, 400, -100000, 400, -8000, 20, -50, 20, -50, 1, -3, 1, -3};
    int i, j, ty;
    int sta[4][17];
...
POINTS chess::seekPoints(int board[15][15])
{
    ...
    for (int i = 0; i < 15; ++i)
    {
        for (int j = 0; j < 15; ++j)
        {
            ...
            {
                board[i][j] = C_WHITE;
                score[i][j] = evaluate(board).score;
                board[i][j] = C_EMPTY;
            }
        }
    }
...
QPoint chess::highestScorePos(int board[15][15])
{
    ...
    for (int i = 0; i < 15; ++i)
    {
        for (int j = 0; j < 15; ++j)
        {
            if (board[i][j] == C_EMPTY) {
                ...
                copyBoard[i][j] = C_WHITE;
                int tmpScore = evaluate(copyBoard).score;
